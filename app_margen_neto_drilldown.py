...
st.subheader("Explorador por ESPECIE")
...
cpareto1, cpareto2, _ = st.columns([2, 2, 6])
cpareto1.metric("Especies TOP (Pareto 80%)", f"{top80_count}")
cpareto2.metric("Utilidad cubierta", f"{top80_pct*100:,.2f}%")

# CSS para que un st.button se vea como tarjeta
st.markdown(
    """
    <style>
      div.stButton > button {
...
for i, row in esp.iterrows():
    with cols[i % 5]:
        ...
        # Si es top 80, envolvemos en un contenedor con clase para CSS
        if bool(row["top_80"]):
            st.markdown('<div class="top80btn">', unsafe_allow_html=True)
